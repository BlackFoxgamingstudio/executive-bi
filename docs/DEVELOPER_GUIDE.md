# Developer Guide: Sovereign Executive BI
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/executive-bi.git
cd executive-bi
...
```bash
export SBB_DATA_SOURCE_URL=your_value_here
export SBB_KPI_DEFINITIONS_PATH=your_value_here
export SBB_ANOMALY_SENSITIVITY=your_value_here
export SBB_REPORT_SCHEDULE=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-executive-bi)
...
```bash
# Build
docker build -t sovereign-executive-bi:dev .

# Run with env file
docker run --env-file .env -p 8790:8790 sovereign-executive-bi:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-executive-bi
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8790`

### Triggering Directly

```bash
curl -X POST http://localhost:8790/api/v1/execute \
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8790` in use | `lsof -i :8790` | Kill PID or change `SERVICE_PORT` |
